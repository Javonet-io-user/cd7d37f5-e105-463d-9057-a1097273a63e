...
public class AndAllCondition<T> implements ICondition<T> {
  ...
  public AndAllCondition(List<ICondition<T>> conditions, Class<?> T) {
    try {
      javonetHandle =
          Javonet.getType("Aspose.Tasks.Util.AndAllCondition`1", getReturnObjectName(T)).create();
    } catch (JavonetException _javonetException) {
      _javonetException.printStackTrace();
    }
  }

  public AndAllCondition(NObject handle) {
