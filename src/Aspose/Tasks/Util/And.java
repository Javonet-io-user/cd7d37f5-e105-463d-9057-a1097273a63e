...
public class And<T> implements ICondition<T> {
  ...
  public And(ICondition<T> cond1, ICondition<T> cond2, Class<?> T) {
    try {
      javonetHandle = Javonet.getType("Aspose.Tasks.Util.And`1", getReturnObjectName(T)).create();
    } catch (JavonetException _javonetException) {
      _javonetException.printStackTrace();
    }
  }

  public And(NObject handle) {
